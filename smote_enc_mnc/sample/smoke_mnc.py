...
import pandas as pd
import numpy as np
from sklearn.utils import check_array, sparsefuncs_fast, check_random_state
...
class SMOTEMNC(SMOTE):
    ...
    def chk_neighbors(self, nn_object, additional_neighbor, metric_params=None):
        if isinstance(nn_object, Integral):
            return NearestNeighbors(n_neighbors=nn_object + additional_neighbor, metric="mahalanobis", metric_params=metric_params, n_jobs=self.n_jobs)
...
#         X_cat_encoded = np.ravel(np.array(X_cat_encoded))
        ...
        for class_sample, n_samples in self.sampling_strategy_.items():
            ...
            inverse_cm = np.linalg.inv(cov_matrix)
            # print(f"cov_matrix, {cov_matrix}, inverse_cm {inverse_cm}")
            nn_k_ = self.chk_neighbors(
                self.k_neighbors, 1, metric_params={'VI': inverse_cm})  # (5, 1)?
            # print("nn_k", nn_k_)
